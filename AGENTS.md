...
- **Rust modules**: `capture.rs` (DXGI), `hotkeys.rs` (global hotkeys), `overlay.rs` (window resize/show/hide + monitor enum), `clipboard.rs`, `save.rs` (validated PNG export to the persisted save folder), `settings.rs` (user prefs persistence), `upload.rs` (→ sinister.ovh)
